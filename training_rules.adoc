...
Unpadding is allowed as an offline preprocessing step.  Unpadding must not change the global data ordering.  The unpadded data set must still follow the rules as the original data set.  For example, globally sorting the padded data set is not allowed, so globally sorting the unpadded data set is also not allowed.

Packing is allowed as an offline preprocessing step, provided (a) Packs have similar distribution to the global data distribution (for example, you cannot have all the shortest sequences in a pack, since that would be similar to sorting), (b) convergence is not affected, (c) randomness for packing is deterministic for a given random seed, and (d) hyperparameter borrowing is still possible, meaning that same set of hyperparameters should converge similarly with or without packing.  The packing algorithm and convergence proof should be submitted to and reviewed by the submitter's working group by N (N=2?) months prior to submission.
...
CLOSED: The same preprocessing steps as the reference implementation must be used, with the exception of batch shuffling for DLRM, in which case the convergence curve needs to match the reference.
