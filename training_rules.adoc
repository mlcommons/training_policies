...
|bert |lamb |global_batch_size |unconstrained |The glboal batch size for training. |--train_batch_size
 ...
 |llama2_70b_lora |adamw |opt_gradient_clip_norm |unconstrained | Gradients are clipped above this norm threshold. |See PR (From Habana, TODO Link)
 ...
 |llama2_70b_lora |adamw |lora_alpha |unconstrained | scaling factor for the LoRA weight matrices |See PR (From Habana, TODO Link)
 |llama2_70b_lora |adamw |opt_weight_decay |unconstrained |weight decay |See PR (From Habana, TODO Link)
...
| GPT3 | Training samples starting from 0 (integer)
