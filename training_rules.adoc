...
|===
 ...
 |qwen35_397b_grpo |adamw |grpo.use_dynamic_sampling |false |dynamic sampling filters out prompt groups with zero reward variance and samples additional prompt groups until the training batch is filled; it must be disabled so that zero-advantage groups are retained rather than discarded and replaced |link:https://github.com/NVIDIA-NeMo/RL/blob/3fca04c9b313d313302923a5bb6b0c8dc0340ed6/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference code] |v6.1
...
** *All eligible generated sequences must be processed by policy training.* Every completed training sequence that is otherwise eligible under the benchmark rules must be included in the policy-training batch and processed by the policy forward pass and loss computation. Submissions must not skip, remove, replace, resample, or otherwise avoid processing a sequence based on its actual or expected loss or gradient contribution (e.g., because its computed advantage is zero, because all rollouts in its group receive the same reward, or because trajectory importance sampling—including sequence-level masking—sets its loss contribution to zero), or based on any equivalent signal or mechanism. Permitted advantage and importance-sampling masks may reduce a sequence's gradient contribution to zero, but they must not be used to remove the sequence before policy training or reduce the number of sequences presented to the policy-training computation. Filtering zero-advantage groups is specifically prohibited because their frequency is an artifact of the benchmark's fixed dataset and limited rollout count and does not reflect typical training conditions.
