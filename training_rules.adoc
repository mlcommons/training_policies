...
The following table lists the tunable hyperparameters for each model,optimizer combination. The value of each tunable hyperparameter must me the listed constraint. 

The MLPerf verifier scripts checks all hyperparameters except those with names marked with asterisks, which must be checked manually. If the verifier and the constraints in this table differ, the verifier is the source of truth.
