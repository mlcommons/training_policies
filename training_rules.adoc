...
Widely-used software frameworks and libraries must be benchmarked using numbered releases, not nightly builds or special benchmarking  builds. However, the selected frameworks and libraries may be built from source to be mutually compatible. For instance Framework version X may be built from source to be compatible with Library version Y and the reverse.

If you are measuring the performance of a publicly available and widely-used system, you must use publicly available and widely-used used versions of the system.
