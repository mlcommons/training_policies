...
Unpadding or packing are both allowed as offline preprocessing steps. When choosing packing: (a) Full flexibility is permitted to reorder the dataset for maximum compression. (b) For benchmarks where such an pre-processing step is an option, the final target should include both, a quality metric and a minimum number of steps that the optimization must be run for. This is to guard against a potential super-convergence coming from the freedom to re-order the dataset. (c) Randomness for packing should be deterministic for a given random seed. (d) The packing algorithm and convergence proof should be submitted to and reviewed by the relevant working group N (N = 1) month before submission.
...
Where data pipelines randomly order data, arbitrary sharding, batching, and packing are allowed provided that (1) the data is not ordered to improve convergence and (2) each datum still appears exactly once.
