...
** *Filtering of zero-advantage samples is prohibited.* Submissions must not skip, filter, or otherwise avoid processing training samples on the basis that their computed advantage is zero (e.g., when all rollouts in a group receive the same reward). All training samples otherwise eligible for the policy training step must be processed regardless of their advantage values. This optimization is disallowed because the frequency of zero-advantage groups is an artifact of the benchmark's fixed dataset and limited rollout count, and does not reflect typical training conditions.
