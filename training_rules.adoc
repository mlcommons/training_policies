...
Unless part of the definition of a benchmark, the implementation should not encode any information about the content of the dataset or a successful model’s state in any form.

For benchmarks which are defined as starting from a fixed set of weights, the implementation should start from the weights provided in the benchmark reference definition, or if that is not posssible, provide  information and code sufficient for reproducing how those starting weights were obtained.
...
** Steps necessary for reproducing the initial set of weights (for benchmarks which use pretrained weights)
* Model
** Publication/Attribution
** List of layers 
