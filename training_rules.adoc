...
For llm, manipulation of metadata which consists of the number of documents in the dataset and the size of each document is allowed as long as the data tokens are not accessed.
...
| |Large Language Model |GPT-3 |2.69 log perplexity
...
|bert |lamb |global_batch_size |unconstrained |The glboal batch size for training. |--train_batch_size
 ...
 |llm |adam |global_batch_size |unconstrained |batch size in sequences |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_adam_beta_1 |0.9 |adam beta1 |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_adam_beta_2 |0.95 |adam beta2 |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_adam_epsilon |0.1 |adam epsilon |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_gradient_clip_norm |1.0 |Gradients are clipped above this norm threshold. |See PR (From NV and Google, TODO Link)
 |llm |adam |sequence_length |2048 |sequence length |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_weight_decay |0.1 |weight decay |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_gradient_accumulation_steps |unconstrained |Numer of fwd/bwd steps between optimizer step. |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_learning_rate_warmup_steps |ceil(265 * 1536 / global_batch_size) |steps taken for linear warmup during initial checkpoint generation. This only affects the learning rate curve in the benchmarking region. |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_learning_rate_decay_steps |ceil(108600 * 1536 / global_batch_size) |learning rate cosine decay from (opt_learning_rate_warmup_steps + 1,opt_learning_rate_decay_steps]. |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_init_checkpoint_step |ceil(4000 * 1536 / batch_size) |first step after loading initial checkpoint |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_base_learning_rate |constrained based on global_batch_size |refer to next table in section "LLM learning rates" |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_end_learning_rate |10% of opt_base_learning_rate |learning rate at the last step of decay period |See PR (From NV and Google, TODO Link)
...
==== LLM hyperparameter constraints
...
* For (3073,4096) - opt_base_learning_rate=2.0e-5 or opt_base_learning_rate=3.0e-5
...
Since the hyperparameters are fixed for LLM, hyperparameter borrowing is not allowed. 
...
|        |large Language Model |GPT-3| Every 24576 samples. CEIL(24576 / global_batch_size) if 24576 is not divisible by GBS
...
* For LLM where there are two reference implementations which have been verified to be equivalent with minimum variance, each reference implementation should provide at least N epoch convergence numbers for each RCP.
...
* For LLM, a request to generate RCPs should be made in the Training WG meeting at least 9 weeks before submission deadline and both reference owners (NV and Google) should provide RCPs (N runs each) at least 5 weeks before submission deadline so that all submitters have enough time to train with the new hparams. The RCP requests should be handled in FCFS order and if there are more than 5 RCP requests, the WG should decide if the requester's set of convergence points (2N runs) can be used as temporary RCPs.
...
| LLM | Training sample starting from 0 (integer)
