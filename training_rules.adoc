...
|bert |lamb |global_batch_size |unconstrained |The glboal batch size for training. |--train_batch_size
 ...
 |llm |adam |opt_adam_beta_2 |0.995 |adam beta1 |See PR (From NV and Google, TODO Link)
 ...
 |llm |adam |opt_learning_rate_warmup_steps |265 |steps taken for linear warmup during initial checkpoint generation. This only affects the learning rate curve in the benchmarking region. |See PR (From NV and Google, TODO Link)
 |llm |adam |opt_learning_rate_decay_steps |108600 |learning rate cosine decay from opt_learning_rate_warmup_steps + 1 = 266th step until 108600th step. |See PR (From NV and Google, TODO Link)
