...
|        |NLP |BERT| eval_start=1M, eval_interval_samples=FLOOR(0.05*(230.23*GBS+3000000), 25000)
...
For v1.0 only, BERT submissions may use either clip-norm-before-reduce or clip-norm-after-reduce.  However, for at scale BERT submissions, submitters are disallowed from using clip-norm-after-reduce to enable overlap of communication and math optimizations.  For future rounds, the expectation is that we move to clip-norm-after-reduce since the math is more accurate.
