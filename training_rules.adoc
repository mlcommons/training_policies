...
For v0.7, the following rules apply:

CLOSED:

The implementation of the optimizer must match the equation specified in the "MLPerf optimizers review" document.  That document describes  which optimizers in the popular deep learning frameworks are compliant by default.  If a submission uses an alternate implementation, the submitter must describe the optimizer's equation and demonstrate equivalence with the approved optimizers on that list.
