...
Results need to be informative and provide basic information about how they were run and convergence without needing to run additional scripts or consulting conmfigurational files. Specifically, the results the following must be disclosed as a part of each result

* Smallest (most compact) numerical precision used to accelerate training 

* Parallelization techniques employed, such as CP8, PP4, or TP1

* RCP checker output

Additionally, configuration files must be named so that they can easily be matched with output files. 
