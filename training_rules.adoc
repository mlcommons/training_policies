...
=== Dead code
Reference implementation code that does contribute to the input preprocessing, output computation, weight updates, or target accuracy evaluation, such that the reference implementation would produce consume the same inputs and produce the same weights, outputs, and target accuracy score(s) regardless of actual data value without the code in question is consired dead code and may be eliminated. For example, unused accuracy metrics may be eliminated.
