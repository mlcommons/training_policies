...
For v5.0's Llama 3.1 405B benchmark only, we allow submissions that converge **1 evaluation (46080 samples) earlier than allowed by the reference** to submit with correctly computed scaling factor, so that the score will be normalized to the reference RCP mean for a fair comparison. We are introducing this rule since we acknowledge that convergence debuggings can be very difficult for a benchmark at this scale given the limited amount of time. For future rounds, we expect submissions to comply with the original convergence. 
