...
Packing is allowed as an offline preprocessing step, provided (a) Packs have similar distribution to the global data distribution, and (b) convergence is not affected.  The packing algorithm and convergence proof should be submitted N months before the submisions deadline and reviewed by the submitter's working group by M months prior to submission.
...
Any randomness needs to be deterministic for a given seed.
