...
|===
 ...
 |deepseekv3_671b |adamw |max_steps |12000 |maximum number of steps |https://github.com/mlcommons/training/blob/daff3992f09b00326d404eaedf575727ff078170/llm_moe_pretraining/nemo/run_deepseek_v3_671b.sh#L61[reference_code] |v6.0
|===
...
While cherry-picking or unfairly advantaged submissions cannot always be conclusively proven, if it is suspected during the review period — based, for example, on inconsistencies in log files or significant deviations from results obtained on equivalent hardware and software or any other equivalent methodology — the review chairs may request a new set of runs (using the exact same software and hyperparameters as the original submission) or require withdrawal of any result deemed to be in violation or any other equivalent decision that the review commitee deems essential. This rule can be modified based on specific cases during the review period. 
