...
|===
 ...
 |flux.1 |adamw |global_batch_size |>=15360 |The global batch size for training |link: https://github.com/pytorch/torchtitan/blob/3d7ea4b6b7e5cd2decf11a1c37581d1cd3678fe0/torchtitan/config_manager.py#L198[reference_code] |v6.0
 ...
 |deepseekv3_671b |adamw |global_batch_size |unconstrained |batch size in sequences |TODO |v6.0
 |deepseekv3_671b |adamw |opt_adamw_beta_1 |0.9 |AdamW beta1 |TODO |v6.0
 |deepseekv3_671b |adamw |opt_adamw_beta_2 |0.95 |AdamW beta2 |TODO |v6.0
 |deepseekv3_671b |adamw |opt_adamw_epsilon |1e-8 |AdamW epsilon |TODO |v6.0
 |deepseekv3_671b |adamw |opt_gradient_clip_norm |1.0 |Gradients are clipped above this norm threshold. |TODO |v6.0
 |deepseekv3_671b |adamw |opt_adamw_weight_decay |0.1 |weight decay |TODO |v6.0
 |deepseekv3_671b |adamw |opt_learning_rate_warmup_steps |4 |steps taken for linear warmup. |TODO |v6.0
 |deepseekv3_671b |adamw |opt_learning_rate_decay_steps |12000 - opt_learning_rate_warmup_steps |Step when the end of cosine learning rate curve is reached. Learning rate cosine decay is in range (opt_learning_rate_warmup_steps + 1,opt_learning_rate_decay_steps]. |TODO |v6.0
 |deepseekv3_671b |adamw |opt_base_learning_rate |0.000024 * sqrt(global_batch_size / 16384) |base learning rate |TODO |v6.0
 |deepseekv3_671b |adamw |sequence_length |4096 |sequence length |TODO |v6.0
 |deepseekv3_671b |adamw |gradient_accumulation_steps |unconstrained |Number of fwd/bwd steps between optimizer step. |TODO |v6.0
 |deepseekv3_671b |adamw |max_steps |12000 |maximum number of steps |TODO |v6.0
|===
...
To closely follow the training recipe from the https://arxiv.org/html/2412.19437v2[DeepSeek-v3 Technical report] which employs a batch size scheduling strategy, where the batch size is gradually increased from 3072 to 15360 in the training of the first 469B tokens, and then keeps 15360 in the remaining training of 4.8T tokens, the benchmark enforces all submissions to use a GBS of atleast 15360. This ensures that the benchmark is representative of DeepSeek-v3 pretraining while maintaining fairness because the task force experiments showed that convergence in the benchmarking region can be much faster with GBS << 15360. Also, since training large language models is very expensive, the task force aims to limit hyperparameter searches by fixing the max learning rate to be used with each GBS.

* GBS<15360 - not allowed
* GBS>=15360: opt_base_learning_rate = 0.000024 * sqrt(GBS / 16384), rounded to the **9th** decimal place.

If a new learning rate is needed for any GBS point and you cannot run the reference code, request new RCPs from the task force or normalize the score if permissible.

Given the very large GBS, the benchmark is setup to evaluate after each training GBS step (instead of evaluation after a predetermined number of samples are trained like in other MLPerf training benchmarks) by reducing the cost of evaluation (by limiting the number of evaluation samples to just 1024).
