...
Unpadding is allowed as an offline preprocessing step.  Unpadding must not change the global data ordering.

Packing is allowed as an offline preprocessing step, provided (a) Packs have similar distribution to the global data distribution, (b) convergence is not affected, and (c) randomness for packing is deterministic for a given random seed.  The packing algorithm and convergence proof should be submitted to and reviewed by the submitter's working group by N months prior to submission.
