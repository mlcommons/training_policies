...
=== Result files
A valid MLPerf result requires that result log files include the full framework logs and all MLLOG lines. Submitters may redact internal paths and environment variables, or truncate framework logs if the file size exceeds 100 MiB, in accordance with GitHub's file size limit. Because framework logs are not standardized, submission automation may not be able to fully enforce this requirement. During the review process, any submitter can be asked by a peer reviewer or the review committee to provide the complete untruncated run log if it was not already included. Failure to provide the full run log upon request may result in removal of the result at the discretion of the review committee.
