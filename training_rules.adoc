...
While cherry-picking cannot always be conclusively proven, if it is suspected during the review period — based, for example, on inconsistencies in log files or significant deviations from results obtained on equivalent hardware and software — the review chairs may request a new set of runs or require withdrawal of any result deemed to be in violation.
