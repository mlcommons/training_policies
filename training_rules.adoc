...
|===
 ...
 |deepseekv3_671b |adamw |moe_routed_bias_update_rate |1e-3 |The expert bias is updated based on the number of assigned tokens to each expert
    in a global batch, where the bias is increased for the experts with less assigned tokens
    and decreased for the experts with more assigned tokens. |https://github.com/NVIDIA/Megatron-LM/blob/b0cc2706ddc60d2aefd5fff346445b5c013036a8/megatron/core/transformer/transformer_config.py#L639-L643[reference_code] |v6.1
 ...
 |qwen35_397b_grpo |adamw |eval_samples |251 |number of held-out R2E-Gym validation tasks; fixed; pass@4 uses `num_val_generations_per_prompt = 4`, so the resulting number of validation rollouts is 1004 (`251 × 4`) |link:https://github.com/NVIDIA-NeMo/RL/blob/3fca04c9b313d313302923a5bb6b0c8dc0340ed6/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference code] |v6.1
 ...
 |qwen35_397b_grpo |adamw |env.nemo_gym.swe_agents_*.responses_api_agents.swe_agents.swebench_agent_timeout |1800 |maximum agent runtime in seconds for each training or validation rollout; fixed |link:https://github.com/NVIDIA-NeMo/RL/blob/644f3708e34a04afe0ffd36a908ff75e574fb5c1/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference code] |v6.1
 |qwen35_397b_grpo |adamw |env.nemo_gym.swe_agents_*.responses_api_agents.swe_agents.swebench_tests_timeout |60 |maximum R2E-Gym test runtime in seconds for each training or validation rollout; fixed |link:https://github.com/NVIDIA-NeMo/RL/blob/644f3708e34a04afe0ffd36a908ff75e574fb5c1/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference code] |v6.1
 |qwen35_397b_grpo |adamw |env.nemo_gym.swe_agents_*.responses_api_agents.swe_agents.command_exec_timeout |60 |maximum explicit hard timeout in seconds for OpenHands shell commands during training or validation rollouts; fixed |link:https://github.com/NVIDIA-NeMo/RL/blob/644f3708e34a04afe0ffd36a908ff75e574fb5c1/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference code] |v6.1
 ...
 |qwen35_397b_grpo |adamw |env.nemo_gym.swe_agents_*.responses_api_agents.swe_agents.agent_framework_commit |submitter-selected immutable commit of an OpenHands `CodeActAgent`-based harness satisfying the benchmark-specific agent-harness rule |the exact OpenHands repository and commit are not fixed; the repository, commit, and all deviations from the reference must be disclosed; see <<qwen35_openhands_agent_harness,benchmark-specific agent-harness rule>> |link:https://github.com/NVIDIA-NeMo/RL/blob/3fca04c9b313d313302923a5bb6b0c8dc0340ed6/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference code] |v6.1
...
=== Result files
A valid MLPerf result requires that result log files (result_*.txt) include the full framework logs and all MLLOG lines. Submitters may redact internal paths and environment variables, or truncate the logs if the file size exceeds 100 MiB, in accordance with GitHub's file size limit. Because framework specific logging is not standardized, submission automation cannot enforce this requirement. Thus during the review process, any submitter can be asked by a peer reviewer or the review committee to provide the complete untruncated result log file if it was not already included. Failure to provide the untruncated and unmodified log upon request may result in removal of the result at the discretion of the review committee.
...
[[qwen35_openhands_agent_harness]]
* qwen35_397b_grpo (Qwen3.5-397B-A17B)
** *OpenHands agent harness.* The software-engineering agent harness must be based on the reference OpenHands `CodeActAgent` harness. For this rule, the reference harness is link:https://github.com/OpenHands/OpenHands/tree/0d766ad06b2be64a42e6f0175b9ebcc4a06599d9/openhands/agenthub/codeact_agent[OpenHands commit 0d766ad06b2be64a42e6f0175b9ebcc4a06599d9] with the link:https://github.com/NVIDIA-NeMo/RL/blob/3fca04c9b313d313302923a5bb6b0c8dc0340ed6/qwen_35/configs/grpo_qwen35_397b_swe_openhands_async.yaml[reference Qwen3.5 configuration]. A submission's repository and commit need not match the reference. Narrow, documented deviations are allowed, e.g., for framework or server compatibility, reliability and failure handling, resource isolation, or security. Such deviations may affect only behavior necessary for their stated purpose and must not otherwise change the benchmark task or scoring contract. Except for the explicit `security_risk` schema variation below, fully rendered model inputs for equivalent benchmark inputs, including the system and user prompts, included templates, and conversation history, must match the reference during normal successful execution, as must the enabled model-facing tools, turn limit and accounting, and termination behavior.
** *Tool-call schema.* The model-facing tools must be exactly the reference `execute_bash`, `think`, `finish`, `task_tracker`, and `str_replace_editor` tools, in the reference order, with the reference names, descriptions, argument types, enumerations, and required parameters. Serialization and parsing must preserve the reference Qwen3.5 XML and OpenAI-compatible tool-call semantics; a backend-specific parser implementation is allowed only if it is behaviorally equivalent. Subject to the permitted harness deviations above, action and result semantics during normal successful execution must match the reference. As the sole permitted schema variation, a submission may make `security_risk` optional by removing it from the `required` lists of `execute_bash` and `str_replace_editor`. In this benchmark's noninteractive configuration, `security_risk` is auxiliary model-generated metadata rather than an input required for tool execution or task scoring. Making it optional must not otherwise change tool capabilities, execution controls, task scoring, or training-sample filtering rules. When optional, `security_risk` must remain a string property whose model-emitted values are limited to `LOW`, `MEDIUM`, or `HIGH`. If the model omits the property entirely, the harness must accept the otherwise valid tool call; for internal risk handling, the omission is equivalent to `UNKNOWN`. `UNKNOWN` is an internal harness state, not a model-facing value or generated token. Omission must not change action execution or task scoring.
** The submission must identify its OpenHands repository and immutable commit, enumerate every deviation from the reference, and state whether it uses the permitted optional `security_risk` schema variation.
