...
| |remain_steps |Arbitrary constant
| |decay_steps |Arbitrary constant
| |warmup function |May use alt_warmup fn in reference
| |learning_rate_warmup_steps |Arbitary constant, suggest 200
|Transformer |lr hyperparam to learning rate function |Arbitrary constant
...
For version 0.5.1, MLPerf will be moving to a batchsize-to-hyperparameter table. 

OPEN: Hyperparameters may be freely changed.
