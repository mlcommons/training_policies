...
** *Tool-call schema.* The model-facing tools must be exactly the reference `execute_bash`, `think`, `finish`, `task_tracker`, and `str_replace_editor` tools, in the reference order, with the reference names, descriptions, argument types, enumerations, and required parameters. Serialization and parsing must preserve the reference Qwen3.5 XML and OpenAI-compatible tool-call semantics; a backend-specific parser implementation is allowed only if it is behaviorally equivalent. Subject to the permitted harness deviations above, action and result semantics during normal successful execution must match the reference. As the sole permitted schema variation, `security_risk` is an optional string property of `execute_bash` and `str_replace_editor` and must not appear in their `required` lists. If present in model output, its value must be `LOW`, `MEDIUM`, or `HIGH`. If omitted, the harness must accept the otherwise valid tool call and treat the omission as the internal risk state `UNKNOWN`. `UNKNOWN` is not a model-facing value or generated token. In this benchmark's noninteractive configuration, `security_risk` is auxiliary metadata and is not used for tool execution or task scoring.
** The submission must identify its OpenHands repository and immutable commit and enumerate every deviation from the reference.
