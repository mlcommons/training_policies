...
Notes:
* All three numbers M,S,T are chosen by the submitter. This allows the submnitter to accomodate his submission to available machine resources, i.e. compute capacity and compute time.
...
Note: the submitter is allowed to prune this number by removing results from individual training instances. As long as the minimum number of models rule is satisfied (see section <<Benchmark Results>> below), the submission is valid. He then uses a modified number of models M'<=M and computes TTTa over the reduced set. This allows the submitter to remove occasional outliers or stragglers which would otherwise reduce the score disproportionally.
